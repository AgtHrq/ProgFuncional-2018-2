...
(define (filter p lst)
  (define (filter p current lst)
    (cond 
      [(empty? lst) (reverse current)]
      [(p (first lst)) (filter p (cons (first lst) current) (rest lst))]
      [else (filter p current (rest lst))]
    )
  )
  (filter p '() lst)
)
...
(define (filtra-par-lg lst)
  (filter (lambda (x) (verifica-paridade x)) lst)
...
(define (tamanho-lista-strings lstr) 
  (string-length (foldr string-append "" lstr))
)
...
;;
(define (todos? p lst)
  (foldr (lambda (a b) (and a b)) #t (map p lst))
)
...
;;
(define (algum? p lst) 
  (foldr (lambda (a b) (or a b)) #f (map p lst))
)
