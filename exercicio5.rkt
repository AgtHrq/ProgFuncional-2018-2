...
(define (tamanho-str-lista lista-strings) 
  (if (empty? lista-strings)
    '()
    (map (lambda (str) (string-length str))  lista-strings)
  )
)
...
(define (adiciona-prefixo pre lista-assuntos) 
	(if (empty? lista-assuntos)	'()
		(map (lambda (assunto) (string-normalize-spaces (~a "[" pre "] " assunto))) lista-assuntos)
	)
)
...
(define (fold-right f val lst) 
	(if (empty? lst)
		val
		(f (car lst) (fold-right f val (cdr lst)))
	)
)
