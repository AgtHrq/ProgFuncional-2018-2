...
(define (print-vector v)
  (for ([i (in-range (vector-length v))])
    (displayln (vector-ref v i))))                      ;; v[i] = (vector-ref v i)
...
(define (vector-map! f v)
  (for ([i (in-range (vector-length v))])
    (vector-set! v i (*(vector-ref v i) (vector-ref v i))))
...
(define (menor-valor-fold vec)
  (for/fold ([menor 1000000000000])  ;;numero grande aleatório
            ([x vec])
...
(define (maior-string-fold vec)
  (for/fold ([str "a"])
            ([x vec])
    (if (> (string-length x) (string-length str)) (begin (set! str x) str) str)))


;; Escreva testes para essas funções.

(define-test-suite test-questao-5
  (test-equal? "MULTIPLICACAO" (mult-vetor-fold (vector 1 2 3)) 6)
  (test-equal? "MENORVALOR" (menor-valor-fold (vector 3 5 1 10 6)) 1)
  (test-equal? "CONCATENA" (concatena-vetor-fold (vector "oi" "tudo" "bem")) "oitudobem")
  (test-equal? "MAIORSTRING" (maior-string-fold (vector "eu" "te" "amo")) "amo")
  )
...
(define (soma-todos arq)
  
  (define x 0)
  (define (linha-atual arq acc)
    (set! x (read arq))
          (cond [(eof-object? x) acc]
                [else (linha-atual arq (+ acc x))]))
  (linha-atual arq 0)
...
(test-begin
  (test-equal? "Soma todos os arquivos" (soma-todos arq) 510)
 )
