...
(module+ test
  ...
  (define-test-suite test-eval
    ...
    (test-equal? "TESTEBEGIN1" (eval '(begin (+ 4 5))) 9)
    (test-equal? "TESTEBEGIN2" (eval '(begin (+ 2 2) (* 5 5))) 25)
    (test-equal? "TESTEVAL1"   (eval-temp-env '((x 4)) '(+ x 10))14)
    (test-equal? "TESTEVAL2"   (eval-temp-env '((f (fun (n) (* n 5))))'(f (+ 2 2)))20)
    )
